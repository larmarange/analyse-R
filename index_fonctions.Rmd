...
fichiers[fichiers != 'index_fonctions.Rmd']
...
for (f in fichiers) {
  ...
  m <- str_match_all(content, '`([a-zA-Z0-9._%]+)`[{]data-package="([a-zA-Z0-9._]+)"( data-rdocumentation=")?([a-zA-Z0-9._]*)')
  m <- m[[1]]
  if (nrow(m)>0)
...
for (j in 1:nrow(lf)) {
  ...
  if (lf[j, "fonction"] != f | lf[j, "package"] != p) {
    if (nchar(lf[j, "rdocumentation"]) == 0)
      res <- paste0(res, 
        "\n`", lf[j, "fonction"], "`{data-package=\"", 
        lf[j, "package"],"\"} (", lf[j, "package"], ")\n\n"
      )
    else
      res <- paste0(res, 
        "\n`", lf[j, "fonction"], "`{data-package=\"", 
        lf[j, "package"],"\" data-rdocumentation=\"", lf[j, "rdocumentation"],
        "\"} (", lf[j, "package"], ")\n\n"
      )
  }

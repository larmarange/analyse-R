...
(en particulier lorsque l'on cherchera à importer ou exporter des données,
voir le chapitre dédié, MAJ_LIEN),
**R** évaluera le nom du fichier qu'on lui a transmis par rapport au 
*répertoire de travail* actuellement défini, qui correspond au répertoire dans
...
`setwd`{data-package="base" data-rdocumentation="getwd"} ou, sous **RStudio**
via le menu *Session > Set Working Directory*. Cependant, nous allons voir que
nous n'aurons en pratique jamais besoin de le faire, car lorsqu'on travaille avec
**RStudio**, il est préférable d'organiser son travail en *projets*.
...
L'idée principale est de réunir tous les fichiers / documents relatifs à un même projet
dans un répertoire dédié^[Dans lequel il sera possible de créer des sous-répertoires.].
...
déjà existant ou bien (iii) à partir d'un gestionnaire de versions (**Git** ou **SVN**).
...
Nous n'aborderons pas ici les gestionnaires de versions^[Pour une présentation de ces logiciels,
voir <http://fr.wikipedia.org/wiki/Logiciel_de_gestion_de_versions>.] qui sont destinés aux
...
Il est encore un peu tôt pour se lancer dans la création de sa propre extension pour **R**.
Les applications **Shiny**^[Voir <http://shiny.rstudio.com/>.] sont des applications webs
interactives. Là encore, on attendra une meilleure maîtrise de **R** pour se lancer dans
ce type de projets. Dans un contexte d'analyse d'enquêtes, on choisira dès lors 
*Empty project*.
...
Les deux options suivantes ne nous concernent pas ici. **RStudio** nous demande s'il on
veut activer *Git* sur ce projet (**Git** étant un gestionnaire de versions) et s'il on
souhaite utiliser l'extension <strong class="package">packrat</strong> sur ce projet.
<strong class="package">packrat</strong> permet une gestion des extensions utilisées,
...
la fonction `library`{data-package="base"} ne sont pas systématiquement rechargées
...
Au sein d'un même projet, on peut avoir plusieurs scripts **R**. Cela permet
de mieux organiser son code. Par exemple, on pourra avoir un premier script
...
fonction `source`{data-package="base"} à laquelle on précisera le nom de fichier
...
le fichier `preparation.R` sera chargé en mémoire, exécuter, puis l'exécution du fichier
`analyses.R` se poursuivra.

Ici, on a indiqué à `source`{data-package="base"} le fichier `preparation.R` sans mention
de répertoire. Dès lors, **R** va aller chercher ce fichier dans le répertoire de travail.
Sur un gros projet, on peut être amener à organiser ses fichiers en plusieurs sous-répertoires
pour une meilleure lisibilité. Dès lors, il faudra indiquer le *chemin relatif* pour accéder
...
on utilisera toujours de préférence la barre oblique simple (`/`), **R** sachant « retrouver ses
petits » selon le système d'exploitation.

Par ailleurs, la fonction d'autocomplétion de **RStudio** fonctionne aussi pour les noms de fichiers.
Essayons d'appuyer sur la touche <kbd>Tab</kbd> après avoir taper les premières lettres du nom de votre
fichier.
</div>

---
title: "Vectorisation (dont purrr)"
---
...
L'extension `purrr`{.pkg} du *tidyverse* a beaucoup évolué ces dernières années et permet d'itérer facilement des opérations sur une liste ou un tableau. Cette extension s'articule également avec les tableaux imbriqués rendus possibles par `tibble`{.pkg} et `tidyr`{.pkg} (voir la fonction `nest`{data-pkg="tidyr"} et sa vignette dédiée : <https://tidyr.tidyverse.org/articles/nest.html>).

Concernant `purrr`{.pkg}, on pourra consulter le chapitre [Itérer avec purrr](https://juba.github.io/tidyverse/18-purrr.html) de l'excellente *Introduction à R et au tidyverse* de Julien Barnier (<https://juba.github.io/tidyverse>).

Julien Barnier y aborde notamment la fonction `map`{data-pkg="purrr"} et ses variantes comme `map_dbl`{data-pkg="purrr"}, `map_chr`{data-pkg="purrr"}, `map_int`{data-pkg="purrr"}, `map_dfr`{data-pkg="purrr"}, `map_dfc`{data-pkg="purrr"}, `modify`{data-pkg="purrr"}, `imap`{data-pkg="purrr"}, `walk`{data-pkg="purrr"}...

...
La fonction `as_factor`{data-package="haven" data-rdocumentation="labelled"} créera automatiquement un 
facteur à partir des étiquettes.

Une alternative consiste à utiliser la fonction `lfactor`{data-package="JLutils"} de l'extension
<strong class="pkg">JLutils</strong> qui fonctionne également si les données ont été importées
avec `read.spss`{data-package="foreign"} ou `read.dta`{data-package="foreign"} de l'extension
...
variable *age* en groupe d'âges avec la fonction `cut`{data-package="base"}
...
La fonction `glm`{data-package="stats"} (pour <dfn lang="en">generalized linear models</dfn> soit
...
`summary`{data-package="stats" data-rdocumentation="summary.glm"}.
...
L'extension <strong class="pkg">effects</strong> propose une représentation graphique résumant les effets de chaque variable du modèle. Pour cela, il suffit d'appliquer la méthode `plot`{data-package="effects" data-rdocumentation="summary.effect"} au résultat de la fonction `allEffects`{data-package="effects" data-rdocumentation="effect"}. Nous obtenons alors la [figure ci-dessous](#reglog_alleffects).

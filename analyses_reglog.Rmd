...
knitr::opts_chunk$set(fig.path = 'images/analyses_reglog/')
```
...
avec l'extension <strong class="pkg">questionr</strong>.
...
de l'extension <strong class="pkg">questionr</strong> :
...
<div class="info">
Si l'on a importé nos données depuis **SPSS** ou **Stata** avec l'extension <strong class="pkg">haven</strong>,
...
La fonction `as_factor`{data-pkg="haven" data-rdocumentation="labelled"} créera automatiquement un 
facteur à partir des étiquettes.

Une alternative consiste à utiliser la fonction `lfactor`{data-pkg="JLutils"} de l'extension
<strong class="pkg">JLutils</strong> qui fonctionne également si les données ont été importées
avec `read.spss`{data-pkg="foreign"} ou `read.dta`{data-pkg="foreign"} de l'extension
<strong class="pkg">foreign</strong> sans avoir converti les variables labelisées en facteurs.
</div>
...
Une présentation plus complète des résultats est obtenue avec la méthode `summary`{data-pkg="stats" data-rdocumentation="summary.glm"} :
...
et `exp`{data-pkg="base" data-rdocumentation="log"} de calculer l'exponentiel. 
...
`summary`{data-pkg="stats" data-rdocumentation="summary.glm"}.

Si vous disposez de l'extension <strong class="pkg">questionr</strong>, la fonction `odds.ratio`{data-pacakge="questionr"} permet de calculer directement les *odds ratio*, 
...
L'extension <strong class="pkg">effects</strong> propose une représentation graphique résumant les effets de chaque variable du modèle. Pour cela, il suffit d'appliquer la méthode `plot`{data-pkg="effects" data-rdocumentation="summary.effect"} au résultat de la fonction `allEffects`{data-pkg="effects" data-rdocumentation="effect"}. Nous obtenons alors la [figure ci-dessous](#reglog_alleffects).
...
La méthode `predict`{data-pkg="stats" data-rdocumentation="predict.glm"} avec l'argument `type="response"`
...
<strong class="pkg">nnet</strong>^[Une alternative est d'avoir recours à l'extension 

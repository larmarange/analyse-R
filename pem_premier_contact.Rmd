...
L'<em class="concept">interface</em> de **RStudio** est divisée en quatre quadrants :

* le quadrant supérieur gauche est dédié aux différents fichiers de travail (nous y reviendrons
  dans le chapitre [Premier travail avec les données](pem_premier_travail.html)) ;
...
* le quadrant supérieur droit permet de connaître 
    + la liste des objets en mémoire ou <em class="concept">environnement de travail</em>
      (onglet *Environment*)
    + ainsi que l'<em class="concept">historique des commandes</em> saisies 
...
Pour l'heure, concentrons-nous sur la <em class="concept">console</em>, 
...
ligne est appelée l'<em class="concept">invite de commande</em> (ou <em class="concept">prompt</em> en anglais).
...
Que signifie cette commande ? L'opérateur `<-`{data-package="base" data-rdocumentation="assignOps"} est appelé 
<em class="concept">opérateur d'assignation</em><em class="concept" data-index="assignation, opérateur"></em>. 
...
Un ensemble de données de même nature constituent pour **R** un <em class="concept">vecteur</em> 
(en anglais <em class="concept">vector</em>) et se
construit à l'aide d'une fonction nommée `c`{data-package="base"}^[`c`{data-package="base"} est 
...
<em class="concept">règle de recyclage</em><em class="concept" data-index="recyclage, règle"></em> 
(<em class="concept">recycling rule</em>). Ainsi, `c(1,2) + c(4,5,6,7,8)` vaudra l'équivalent 
de `c(1,2,1,2,1) + c(4,5,6,7,8)`.
</div>
...
Cette opération s'appelle l'<em class="concept">indexation</em> d'un vecteur. 
...
une <em class="concept">coloration syntaxique</em>. 
...
les <em class="concept" data-index="fonction">fonctions</em>. 
Une fonction se caractérise de la manière suivante :

* elle a un nom ;
* elle accepte des <em class="concept" data-index="argument">arguments</em> (qui peuvent avoir un nom ou pas) ;
...
visible est la fonction `c`{data-package="base}. Dans la ligne suivante :

```{r}
reponse <- c("Bac+2", "Bac", "CAP", "Bac", "Bac", "CAP", "BEP")
```

on fait appel à la fonction nommée `c`{data-package="base"}, on lui passe en arguments 
...
Ici, la fonction `length`{data-package="base"} nous renvoie le nombre d'éléments du vecteur, 
la fonction `mean`{data-package="base"} nous donne la moyenne des éléments du vecteur et 
fonction `var`{data-package="stats" data-rdocumentation="cor"} sa variance.
...
une <em class="concept">valeur manquante</em>. Celle-ci est symbolisée dans **R** par le code 
`NA`<em class="concept" data-index="NA"></em> :
...
un paramètre supplémentaire à la fonction `mean`{data-package="base"}, nommé `na.rm` :

```{r}
mean(tailles, na.rm = TRUE)
```

Positionner le paramètre `na.rm` à `TRUE` (vrai) indique à la fonction `mean`{data-package="base"}
...
on parle d'<em class="concept">argument nommé<em class="concept">.
...
|:---------|:------------|
| `c`{data-package="base"} | construit un vecteur à partir d'une série de valeurs |
| `length`{data-package="base"} | nombre d'éléments d'un vecteur |
| `mean`{data-package="base"} | moyenne d'un vecteur de type numérique |
| `var`{data-package="stats" data-rdocumentation="cor"} | variance d'un vecteur de type numérique |
| `+`{data-package="base" data-rdocumentation="Arithmetic"}, `-`{data-package="base" data-rdocumentation="Arithmetic"}, `*`{data-package="base" data-rdocumentation="Arithmetic"}, `/`{data-package="base" data-rdocumentation="Arithmetic"} | opérateurs mathématiques de base |
| `ˆ`{data-package="base" data-rdocumentation="Arithmetic"} | passage à la puissance |

On peut rajouter les fonctions de base suivantes :

| Fonction | Description |
|:---------|:------------|
| `min`{data-package="base" data-rdocumentation="Extremes"} | valeur minimale d'un vecteur numérique |
| `max`{data-package="base" data-rdocumentation="Extremes"} | valeur maximale d'un vecteur numérique |
| `sd`{data-package="stats"} | écart-type d'un vecteur numérique |
| `:`{data-package="base" data-rdocumentation="Colon"} | génère une séquence de nombres. `1:4` équivaut à `c(1,2,3,4)` |

### Aide sur une fonction

Il est très fréquent de ne plus se rappeler quels sont les paramètres d'une fonction ou le type de résultat
qu'elle retourne. Dans ce cas on peut très facilement accéder à l'<em class="concept">aide</em>
décrivant une fonction particulière
avec `?`{data-package=="utils" data-rdocumentation=="Question"} ou `help`{data-package="utils"}. Ainsi,
pour obtenir de l'aide sur la fonction `mean`{data-package="base"}, 
...
<div class="note">
L'utilisation du raccourci `?`{data-package=="utils" data-rdocumentation=="Question"} ne fonctionne 
pas pour certains opérateurs comme `*`{data-package="base" data-rdocumentation="Arithmetic"}. 
...
décrites dans le chapitre [Où trouver de l'aide ?](pem_ou_trouver_aide.html).

### Interprétation des arguments

Prenons l'exemple de la fonction `format`{data-package="base"} dont la version de base
...
<em class="concept" data-index="argument non nommé">arguments non nommés</em>, 
...
<em class="concept" data-index="argument nommé">arguments nommés</em>.
...
`format`{data-package="base"} d'arguments additionnels qui seront utilisés dans certains cas
de figure, ou bien d'arguments qui seront transmis à une fonction secondaire appelée par la
fonction principale, ou encore, comme pour le cas de la fonction `c`{data-package="base"},
de la possibilité de saisir un nombre indéfini de données sources.

### Autocomplétion

**RStudio** fournit un outil bien pratique appelé <em class="concept">autocomplétion</em>^[En bon

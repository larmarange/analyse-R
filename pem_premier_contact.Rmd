...
* le quadrant supérieur droit permet de connaître 
    + la liste des objets en mémoire (onglet *Environment*)
...
Pour l'heure, concentrons-nous sur la *console*, c'est-à-dire le quadrant inférieur gauche.
...
précédemment, que vous pouvez alors facilement réexécuter ou modifier une commande particulière.
...
Voir également (en anglais) : <http://www.rstudio.com/ide/docs/using/history>
</div>
...
quelconque à droite et la place dans l'objet indiqué à gauche. La commande pourrait donc se lire 
...
chiffre), les symboles `.` et `_`. Ils doivent impérativement commencer par une lettre. 
...
manière à lui donner la même taille que le plus long, ce qui s'appelle la règle de recyclage 
(*recycling rule*). Ainsi, `c(1,2) + c(4,5,6,7,8)` vaudra l'équivalent de `c(1,2,1,2,1) + c(4,5,6,7,8)`.
</div>
...
De plus, si vous placez le curseur juste après une parenthèse fermante, la parenthèse ouvrante
...
les *fonctions*. Une fonction se caractérise de la manière suivante :
...
décrites dans le chapitre [Où trouver de l'aide ?] MAJ_LIEN.
...
arguments non nommés, il faut fournir également une valeur pour le troisième argument `digits`.

```{r}
format(c(12.3, 5678), TRUE, NULL, 2)
```

Ce n'est pas forcément ce qu'il y a de plus pratique. D'où l'intérêt des *arguments nommés*.
...
**RStudio** fournit un outil bien pratique appelé *auto-complétion*. Saisissez les premières 
...
À l'intérieur des parenthèses d'une fonctionn, vous pouvez utiliser l'auto-complétion
pour retrouver un argument de cette fonction. 

Vous pouvez également utiliser l'auto-complétion pour retrouver le nom d'un objet
que vous avez précédemment créé.

Pour plus de détails, voir la documentation officielle de **RStudio** (MAJ_LIEN).

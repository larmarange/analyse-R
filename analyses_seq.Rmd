...
knitr::opts_chunk$set(fig.path = 'images/analyses_seq/')
```
...
Depuis les années 1980, l'étude quantitative des trajectoires biographiques (*life course analysis*) a pris
...
de séquences, au sein desquelles l'appariement optimal (*optimal matching*) occupe une place 
...
le package <strong class="package">TraMineR</strong>^[<http://mephisto.unige.ch/traminer/>],
...
<strong class="package">TraMineR</strong> propose de nombreuses fonctions pour l'analyse de séquences. 
L'extension <strong class="package">cluster</strong> comprend un certain nombre de méthodes de 
classification automatique^[Pour une présentation plus détaillée, voir le chapitre sur la 
[classification ascendante hiérarchique (CAH)](analyses_cah.html).].
...
qui composent les séquences et on crée un objet « séquence » avec `seqdef`{data-package="TraMineR"} :
...
Dans notre exemple, on va créer avec `seqsubm`{data-package="TraMineR"} une « matrice des coûts de substitution » dans laquelle tous les coûts sont constants et égaux à 2^[Le fonctionnement de l'algorithme d'appariement 
optimal --- et notamment le choix des coûts --- est décrit dans le chapitre 9 du manuel de 
<strong class="package">TraMineR</strong>
...
Ensuite, on calcule la matrice de distance entre les séquences (i.e contenant les « dissimilarités » entre
les séquences) avec `seqdist`{data-package="TraMineR"}, avec un coût d'insertion/suppression (*indel*) 
...
classification ascendante hiérarchique (CAH), qui permet de regrouper les séquences en un certain nombre
de « classes » en fonction de leur proximité :

```{r}
seq.agnes <- agnes(as.dist(seq.om), method = "ward", keep.diss = FALSE)
```

Avec la fonction `plot`{data-package="stats" data-rdocumentation="dendrogram"}, 
il est possible de tracer l'arbre de la classification (dendrogramme). 
...
(agriculteur, étudiant, etc.). Ce graphique s'obtient avec `seqdplot`{data-package="TraMineR" data-rdocumentation="seqplot"} :
...
Les « tapis » (*index plots*), obtenus avec `seqiplot`{data-package="TraMineR" data-rdocumentation="seqplot"},
...
(on trie ici par *multidimensional scaling* à l'aide de la fonction `cmdscale`{data-package="stats"}).
...
[GitHub](https://github.com/larmarange/JLutils), propose une fonction `seq_heatmap`{data-package="JLutils"}
...
`disscenter`{data-package="TraMineR"}, permet de mesurer plus précisément l'homogénéité 
des classes :
...
`seqfplot`{data-package="TraMineR" data-rdocumentation="seqplot"}.
...
On peut aussi visualiser avec `seqmsplot`{data-package="TraMineR" data-rdocumentation="seqplot"}
...
On peut également représenter avec `seqmtplot`{data-package="TraMineR" data-rdocumentation="seqplot"}
...
Ce type de graphique produit par `seqHtplot`{data-package="TraMineR" data-rdocumentation="seqplot"}

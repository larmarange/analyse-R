...
<div class="webin-R">
Ce chapitre est évoqué dans le webin-R #15 (analyse de survie) sur [YouTube](https://youtu.be/3XLx1SHc2fw).
...
**ATTENTION :** les étiquettes de valeurs sont le plus souvent perdus lors des fusions de tables avec `dplyr`{.pkg}. Pour les récupérer après fusion, nous allons conserver une version originale du tableau de données *enfants* et utiliser la fonction `copy_labels_from`{data-pkg="labelled"} de l'extension `labelled`.
...
Pour représenter ces rapports de risque, on peut ici encore avoir recours à la fonction `ggcoef_model`{data-pkg="GGally"} de l'extension `GGally`{.pkg}.
...
```
<figcaption>Coefficients du modèle avec ggcoef_model</figcaption>
</figure>

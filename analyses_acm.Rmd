...
knitr::opts_chunk$set(fig.path = 'images/analyses_acm/')
```
...
portant sur deux variables qualitatives et l'*analyse des correspondances multiples* (ACM) portant 
...
d'avoir recours à deux autres extensions plus complètes, <strong class="package">ade4</strong>
et <strong class="package">FactoMineR</strong>, chacune ayant ses avantages et des possibilités différentes. Voici les fonctions les plus fréquentes :

| Analyse | Variables | Fonction standard | Fonction <strong class="package">ade4</strong> | Fonctions <strong class="package">FactoMineR</strong> |
|:--------|:----------|:-----------------:|:----------------------------------------:|:--------:|
| ACP | plusieurs variables quantitatives | `princomp`{data-package="stats"} | `dudi.pca`{data-package="ade4"} | `PCA`{data-package="FactoMineR"} |
| AFC | deux variables qualitatives | `corresp`{data-package="MASS"} | `dudi.coa`{data-package="ade4"} | `CA`{data-package="FactoMineR"} |
| ACM | plusieurs variables qualitatives | `mca`{data-package="MASS"} | `dudi.acm`{data-package="ade4"} | `MCA`{data-package="FactoMineR"} |
| Analyse mixte de Hill et Smith | plusieurs variables quantitatives et/ou qualitatives | --- | `dudi.mix`{data-package="ade4"} | --- |
...
à interpréter que de lire directement le fichier de données. On ne voit pas es corrélations qu'il peut y
...
aux effectifs faibles, aussi regrouper les classes quand cela est nécessaire.

## ACM avec ade4

Si l'extension <strong class="package">ade4</strong> n'est pas présente sur votre PC, 
...
<strong class="package">questionr</strong>.
...
En premier lieu, comme dans le [chapitre sur la régression logistique](analyses_reglog.html), 
...
`screeplot`{data-package="ade4" data-rdocumentation="scatter"} :
...
`summary`{data-package="ade4" data-rdocumentation="dudi"}^[On pourra également avoir recours à la 
fonction `inertia.dudi`{data-package="ade4"} pour l'ensemble des axes.] :

```{r}
summary(acm)
```

L'inertie totale est de 1,451 et l'axe 1 en explique 0,1474 soit 17 %. L'inertie projetée cumulée nous
...
corrélation des modalités. Pour cela, on aura recours à `s.corcicle`{data-package="ade4"}. On
...
On pourra avoir également recours à `boxplot`{data-package="ade4" data-rdocumentation="dudi.acm"}
pour visualiser comment se répartissent les modalités
de chaque variable sur un axe donné^[La fonction `score`{data-package="ade4" data-rdocumentation="score.acm"}
...
fonction `barplot`{data-package="graphics"} ainsi :
...
<div class="astuce">
Le paramètre `mfrow` de la fonction `par`{data-package="graphics"} permet d'indiquer à **R**
...
Pour représenter, les modalités dans le plan factoriel, on utilisera la fonction 
`s.label`{data-package="ade4"}. Par défaut, les deux premiers axes sont représentés.
...
(une taille nulle pour les étiquettes), `s.label`{data-package="ade4"} remplace chaque observation
par un symbole qui peut être spécifié avec `pch`^[Voir le chapitre sur les graphiques pour la liste des différents symboles utilisables. (MAJ LIEN)].
...
`s.label`{data-package="ade4"} ne permettra pas de rendre compte les effectifs de chaque point.

Le package <strong class="package">JLutils</strong>, disponible seulement sur 
[GitHub](https://github.com/larmarange/JLutils), propose une petite fonction `s.freq`{data-package="JLutils"}
...
La fonction `s.freq`{data-package="JLutils"} s'emploie de manière similaire aux autres fonctions
graphiques de <strong class="package">ade4</strong>. Le paramètre `csize` permet d'ajuster 
...
La fonction `s.value`{data-package="ade4"} permet notamment de représenter un troisième axe factoriel. 
...
`s.arrow`{data-package="ade4"} permet de représenter les vecteurs variables ou les vecteurs individus 
...
`s.hist`{data-package="s.hist"} permet de représenter des individus (ou des modalités) sur le plan 
...
`s.class`{data-package="ade4"} et `s.chull`{data-package="ade4"} permettent de représenter 
les différentes observations classées en plusieurs catégories.
Cela permet notamment de projeter certaines variables. 

`s.class`{data-package="ade4"} représente les observations par des points, lie chaque 
...
`s.chull`{data-package="ade4"} représente les barycentres de chaque catégorie et dessine
...
lisible. Si vous avez installé l'extension <strong class="package">RColorBrewer</strong>, vous pouvez 
utiliser les différentes palettes de couleurs proposées. 
Pour afficher les palettes disponibles, utilisez 
`display.brewer.all`{data-package="RColorBrewer" data-rdocumentation="RColorsBrewer"}.
...
Pour obtenir une palette de couleurs, utilisez la fonction 
`brewer.pal`{data-package="RColorBrewer" data-rdocumentation="RColorsBrewer"} avec les arguments
...
La variable catégorielle transmise à `s.class`{data-package="ade4"} ou `s.chull`{data-package="ade4"}
...
Les fonctions `scatter`{data-package="ade4"} et 
`biplot`{data-package="ade4" data-rdocumentation="scatter"} sont équivalentes :
elles appliquent `s.class`{data-package="ade4"} à chaque variable utilisée pour l'ACM.
...
Comme avec <strong class="package">ade4</strong>, il est nécessaire de préparer les données au préalable 
(voir section précédente).

L'ACM se calcule avec la fonction `MCA`{data-package="FactoMineR"}, l'argument `ncp` permettant de choisir
...
En premier lieu, il apparait que l'inertie totale obtenue avec `MCA`{data-package="FactoMineR"} est 
différente de celle observée avec `dudi.acm`{data-package="ade4">. Cela est dû à un traitement différents 
des valeurs manquantes. Alors que `dudi.acm`{data-package="ade4"} exclu les
valeurs manquantes, `MCA`{data-package="FactoMineR"} les considèrent, par défaut, comme une modalité
additionnelle. Pour calculer l'ACM uniquement sur les individus n'ayant pas de valeur manquante, on aura recours à `complete.cases`{data-package="stats"} :
...
Les possibilités graphiques de <strong class="package">FactoMineR</strong> sont différentes de celles de 
<strong class="package">ade4</strong>. Un recours à la fonction 
`plot`{data-package="FactorMineR" data-rdocumentation="plot.MCA"} affichera par défaut les individus, 
...
La fonction `plotellipses`{data-package="FactoMineR"} trace des ellipses de confiance atour des 
...
La fonction `dimdesc`{data-package="FactoMineR"} aide à décrire et interpréter les dimensions de l'ACM. 

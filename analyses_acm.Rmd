...
Il existe plusieurs techniques d'<dfn>analyse factorielle</dfn><dfn data-index="factorielle, analyse"> dont 
...
directement à `dudi.acm`{data-package="ade4"} de vous renvoyer les cinq premiers axes ainsi :
...
Pour cela, on aura recours à `s.corcicle`{data-package="ade4"}. On
...
on utilisera la fonction `s.label`{data-package="ade4"}. Par défaut, les deux premiers axes sont représentés.
...
**[GitHub](https://github.com/larmarange/JLutils)**, propose une fonction `s.freq`{data-package="JLutils"}
...
Alors que `dudi.acm`{data-package="ade4"} exclu les

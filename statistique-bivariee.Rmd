...
La fonction `tbl_summary`{data-pkg="gtsummary"} de l'extension `gtsummary`{.pkg} accepte un argument `by` permettant de réaliser des croisements selon une variable catégorielle.
...
Il est possible d'ajouter une colonne total avec `add_overall`{data-pkg="gtsummary"} et des tests statistiques avec `add_p`{data-pkg="gtsummary"}.
...
Citons également la variante `tbl_cross`{data-pkg="gtsummary"} utilisable pour un tableau croisé de seulement deux variables (une en ligne et une en colonne).
...
L'extension `GDAtools`{.pkg} développée par Nicolas Robette propose, entre autres, plusieurs fonctions pour décrire les associations statistiques entre deux variables catégorielles et/ou continues.
...
Pour deux variables catégorielles, on aura recours à , `assoc.twocat`{data-pkg="GDAtools"}, `ggassoc_crosstab`{data-pkg="GDAtools"} et `ggassoc_phiplot`{data-pkg="GDAtools"}.

```{r}
library(GDAtools)
assoc.twocat(d$sexe, d$cuisine)
...
Pour une variable catégorielle croisée avec une variable continue, on aura recours à , `assoc.catcont`{data-pkg="GDAtools"} et `ggassoc_boxplot`{data-pkg="GDAtools"}.

```{r}
assoc.catcont(d$sport, d$heures.tv)
ggassoc_boxplot(d, aes(x = sport, y = heures.tv))
```

Pour deux variables continues, on aura recours à , `assoc.twocont`{data-pkg="GDAtools"} et `ggassoc_scatter`{data-pkg="GDAtools"}.

```{r}
assoc.twocont(d$age, d$heures.tv)
ggassoc_scatter(d, aes(x = age, y = heures.tv))
```

...
knitr::opts_chunk$set(fig.path = 'images/analyses_cah/')
```
...
classes ou sous-groupes. La *classification ascendante hiérarchique* (CAH) est l'une d'entre elles. On cherche
...
préalable qui s'exprimera sous la forme d'une *matrice de distances*, exprimant la distance existant entre
...
individus de manière itérative afin de produire un *dendrogramme* ou *arbre de classification*. La classification
...
à une certaine hauteur choisie, on produira la partition désirée.

<div class="info">
On trouvera également de nombreux supports de cours en français sur la CAH sur le site de François
Gilles Carpentier : <http://geai.univ-brest.fr/~carpenti/>.
</div>

## Calculer une matrice des distances

La notion de *ressemblance* entre observations est évaluée par une distance entre individus. Plusieurs
type de ditances existent selon les données utilisées.
...
La plupart peuvent être calculées avec la fonction `dist`{data-package="stats"}.

Usuellement, pour un ensemble de variables qualitatives, on aura recours à la distance du &Phi;² qui est
celle utilisée pour l'analyse des correspondances multiples (voir le [chapitre dédié](analyses_acm.html)).
Avec l'extension <strong class="package">ade4</strong>, la distance du &Phi;² s'obtient avec la fonction 
`dist.dudi`{data-package="ade4"}^[Cette même fonction peut aussi être utilisée pour calculer une distance 
après une analyse en composantes principales ou une analyse mixte de Hill et Smith.].
Le cas particulier de la CAH avec l'extension <strong class="package">FactoMineR</strong> sera abordée dans 
une section spécifique (MAJ LIEN). Nous évoquerons également la *distance de Gower* qui peut s'appliquer à
un ensemble de variables à la fois qualitatives et quantitatives et qui se calcule avec la fonction 
`daisy`{data-package="cluster"} de l'extension <strong class="package">cluster</strong>. Enfin, dans le 
chapitre sur l'analyse de séquences (MAJ LIEN), nous verrons également la fonction 
`seqdist`{data-package="TraMineR"} (extension <strong class="package">TraMineR</strong>) permettant de
...
l'indice vaut 1, les deux individus sont identiques. À l'opposé, s'il vaut 0, les deux individus considérés
n'ont pas de point commun. Si l'on note *S~g~* l'indice de similarité de Gower, la distance de Gower *D~g~*
s'obtient simplement de la manière suivante : *D~g~* = 1-*S~g~*. Ainsi, la distance sera nulle entre deux individus
identiques et elle sera égale à 1 entre deux individus totalement différents. Cette distance s'obtient sous
**R** avec la fonction `daisy`{data-package="cluster"} du package <strong class="package">cluster</strong>.
...
individus. *s*~12*j*~ représente la similarité partielle entre les individus 1 et 2 concernant le descripteur *j*.
Cette similarité partielle se calcule différemment s'il s'agit d'une variable qualitative ou quantitative :
...
* **variable quantitative :** la différence absolue entre les valeurs des deux variables est tout d'abord
  ...
  noté *R*~*j*~. Dès lors, la similarité partielle vaut *S*~12*j*~ = |*y*~1*j*~ − *y*~2*j*~| / *R*~*j*~.
...
Si l'on supprime individus ayant des valeurs manquantes, 2 est retirée du fichier d'observations et aucune
distance n'est calculée. 
...
de la distance du &chi;². Nous considérons ici que nous avons *Q* questions (soit *Q* variables initiales de type
facteur). À chaque individu est associé un *patron* c'est-à-dire une certaine combinaison de réponses aux
...
Nous allons reprendre l'ACM calculée avec `dudi.acm`{data-package="ade4"} (<strong class="package">ade4</strong>)
dans le [chapitre consacré à l'ACM](analyses_acm.html) :
...
La matrice des distances s'obtient dès lors avec la fonction `dist.dudi`{data-package="ade4"} :
...
les deux individus les plus proches revient alors à choisir la paire de points dont l'agrégation entraîne
la diminution minimale de l'inertie du nuage. En résumé, cette méthode cherche à minimiser l'inertie
intra-classe et à maximiser l'inertie inter-classe afin d'obtenir des classes les plus homogènes possibles.
...
La fonction de base pour le calcul d'un dendrogramme est `hclust`{data-package="stats"} en précisant 
le critère d'aggrégation avec `method`. Dans notre cas, nous allons opter pour la méthode de Ward 
appliquée au carré des distances (ce qu'on indique avec `mdˆ2`) :
...
L'extension <strong class="package">flashClust</strong> permet de réduire significativement le temps de calcul.
Il suffit d'installer puis d'appeler cette extension. La fonction 
`hclust`{data-package="flashClust" data-rdocumentation="flashClust"} sera automatiquement remplacée par cette 
version optimisée :
...
Le dendrogramme obtenu peut être affiché simplement avec `plot`{data-package="stats" data-rdocumentation="hclust"}.
...
La fonction `agnes`{data-package="cluster"} de l'extension <strong class="package">cluster</strong> peut également 
être utilisée pour calculer le dendrogramme. Cependant, à l'usage, elle semble être un peu plus lente que 
`hclust`{data-package="stats"}.
...
Le résultat obtenu n'est pas au même format que celui de `hclust`{data-package="stats"}. Il est possible de 
transformer un objet `agnes`{data-package="cluster" data-rdocumentation="agnes.object"} au format 
`hclust`{data-package="stats"} avec `as.hclust`{data-package="cluster" data-rdocumentation="agnes.object"}.
...
Pour obtenir une partition de la population, il suffit de découper le dendrogramme obtenu à une
...
La fonction `rect.hclust`{data-package="stats"} permet de visualiser les différentes partitions 
...
L'extension <strong class="package">FactoMineR</strong> (que nous aborderons 
[dans une section dédiée ci-après](#factominer)) suggère d'utiliser la partition 
ayant la plus grande perte relative d'inertie. 

Dans l'extension <strong class="package">JLutils</strong> 
(disponible sur [GitHub](https://github.com/larmarange/JLutils)), nous avons développer une fonction 
`best.cutree`{data-package="JLutils"} qui permet de calculer cette indicateur à partir de n'importe 
quel dendrogramme calculé avec `hclust`{data-package="stats"} ou `agnes`{data-package="cluster"}.

Pour installer <strong class="package">JLutils</strong>, on aura recours au package 
...
Par défaut, `best.cutree`{data-package="JLutils"} regarde quelle serait la meilleure partition entre 3 
et 20 classes.
...
un découpage en cinq classes. Le découpage s'effectue avec la fonction `cutree`{stats}.
...
La typologie obtenue peut être représentée dans le plan factoriel avec `s.class`{data-package="ade4"}.
...
Romain François a developpé une fonction `A2Rplot`{data-package="JLutils"} permettant de réaliser 
...
<strong class="package">JLutils</strong>.
...
```
<figcaption>Un dendrogramme coloré</figcaption>
</figure>

## CAH avec l'extension FactoMineR{#factominer}

L'extension <strong class="package">FactoMineR</strong> fournit une fonction `HCPC`{data-package="FactoMineR"}
permettant de réaliser une classification hiérarchique à partir du résultats d'une analyse factorielle réalisée 
avec la même extension (voir la [section dédiée du chapitre sur l'ACM](analyses_acm.html#factominer)).

`HCPC`{data-package="FactoMineR"} réalise à la fois le calcul de la matrice des distances, 
du dendrogramme et le partitionnement de la population en classes. 
Par défaut, `HCPC`{data-package="FactoMineR"} calcule le dendrogramme à partir du carré des distances du &Phi;² 
...
grande perte relative d'inertie (comme avec `best.cutree`{data-package="JLutils"}). Utilisez `graph=FALSE`
...
cah <- HCPC(acm2)
```
<figcaption>Graphique produit par HCPC</figcaption>
</figure>

On pourra représenter le dendrogramme avec `plot`{data-package="FactoMineR" data-rdocumentation="plot.HCPC"}
...
par `dist.dudi`{data-package="ade4"}. Cela est dû au fait que `HCPC`{data-package="FactoMineR"}
...
`HCPC`{data-package="FactoMineR"} n'a donc pris en compte que ces 5 premiers axes pour calculer les 
...
De son côté, `dist.dudi`{data-package="ade4"} prends en compte l'ensemble des axes pour calculer la matrice 
des distances. On peut reproduire cela avec <strong class="package">FactoMineR</strong> en indiquant 
...
L'objet renvoyé par `HCPC`{data-package="FactoMineR"} contient de nombreuses informations. 

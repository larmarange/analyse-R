...
chapitre sur l'[analyse de séquences](analyses_seq.html), nous verrons également la fonction 
...
qui porte son nom. L'objectif de cet indice consiste à mesurer dans quelle mesure deux individus sont semblables.
...
on privilégiera le plus souvent la <dfn>méthode de Ward</dfn><dfn data-index="Ward, méthode"></dfn>.
Cette méthode se distingue de toutes les autres en
...
3.1 de **R**.], l'option `method = "ward.D"` correspondant à la version « classique ») :
...
L'extension <strong class="package">fastcluster</strong> permet de réduire significativement le temps de calcul.
Il suffit d'installer puis d'appeler cette extension. La fonction 
`hclust`{data-package="fastcluster"} sera automatiquement remplacée par cette 
...
ATTENTION : la méthode implémentée dans la fonction `agnes`{data-package="cluster"} correspond à l'option
`method = "ward.D2"` de `hclust`{data-package="stats"}.
...
L'extension <strong class="package">JLutils</strong> 
...
un découpage en cinq classes. Le découpage s'effectue avec la fonction `cutree`{data-package="stats"}.
...
On pourra aussi noter l'extension <strong class="package">ggdendro</strong> pour représenter
des dendrogrammes avec <strong class="package">ggplot2</strong> ou encore l'extension
<strong class="package">dendextend</strong> qui permet de manipuler, représenter et
comparer des dendrogrammes^[Pour une présentation succincte, voir par exemple 
<http://www.r-bloggers.com/dendextend-version-1-0-1-user2015-presentation/> (en anglais).].
</div>

...
On ne peut afficher directement les proportions avec `stat_count`{data-pkg="ggplot2"}. Cependant, l'extension `JLutils`{.pkg}^[Cette extension n'est disponible que sur [GitHub](https://github.com/larmarange/JLutils). On peut l'installer avec la commande suivante : `source("https://install-github.me/larmarange/JLutils")`] fournit une stat `stat_prop`{data-pkg="JLutils"} dont le dénominateur peut être personnalisé via l'esthétique `by` (attention : l'esthétique attendue doit impérativement être un facteur).

```{r, message=FALSE}
library(JLutils)
d$qualreg <- addNAstr(factor(d$qualreg))
ggplot(d) +
  aes(x = qualreg, fill = sport) +
  geom_bar(position = "fill") +
...
p <- ggplot(d) +
  aes(
    x = qualreg, fill = sexe, by = sexe, y = ..prop..,
    label = scales::percent(..prop.., accuracy = 1), 
  ) +
  geom_bar(stat = "prop", position = position_dodge(.9)) +
  geom_text(
    aes(y = ..prop.. - .005), stat = "prop", 

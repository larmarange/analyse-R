...
Selon les résultats de notre modèle, les hommes pratiquent plus un sport que les femmes et la pratique du sport diminue avec l'âge. Pour représenter les effets différentes variables, on peut avoir recours à la fonction `allEffects`{data-pkg="effects" data-rdocumentation="effects"} de l'extension <strong class="pkg">effects</strong>.
...
Jetons maintenant un oeil aux coefficients du modèle. Pour rendre les choses plus visuelles, nous aurons recours à `ggcoef`{data-pkg="GGally"} de l'extension <strong class="pkg">GGally</strong>.
...
```{r include=FALSE}
# Custom version of broken
broken2.glm <- function(model, new_observation, ..., baseline = 0) {
  #ny <- predict.glm(model, newdata = new_observation, type = "terms")
  ny <- betas(model, new_observation)
  terms <- NULL
...
On pourra en particulier se référer à la vignette inclue avec l'extension <strong class="pkg">phia</strong> : <https://cran.r-project.org/web/packages/phia/vignettes/phia.pdf>.

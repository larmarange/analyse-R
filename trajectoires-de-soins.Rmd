...
Note : pour afficher les proportions sur le graphique, aurons recours recours à la statistique `stat_prop`{data-pkg="JLutils"} de l'extension `JLutils`{.pkg}^[Cette extension n'est disponible que sur [GitHub](https://github.com/larmarange/JLutils). On peut l'installer avec la commande suivante : `source("https://install-github.me/larmarange/JLutils")`].
...
Pour mieux visualiser les relations entre les variables, on peut avoir recours à la fonction `ggcross`{data-pkg="JLutils"} de l'extension `JLutils`{.pkg} qui permets de représenter les <dfn data-index="résidus, test du Chi²">résidus du Chi²</dfn><dfn data-index="test du Chi², résidus"></dfn><dfn data-index="Chi², résidus">.
...
ggcross(
  ...
  labels = "percent0(col.prop)",
  fill_breaks = c(-Inf, -4, -2, 0, 2, 4, Inf),
  fill_title = "Résidus de Pearson",
  size = NULL
) +
  theme_minimal()
```

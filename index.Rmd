...
Si vous débutez avec **R** et **RStudio**, nous vous conseillons de parcourir **analyse-R**
dans l'ordre suivant :

1. Manipuler > Prise en main
2. Analyser > Statistiques introductives
3. Manipuler > Manipulations de données (intermédiaire)
4. Analyser > Statistiques intermédiares
5. Manipuler > Manipulations de données (avancé)
6. Analyser > Statistiques avancées
7. Approfondir
...
urls <- c(urls, paste0(base_url, "index_concepts.html"))
urls <- c(urls, paste0(base_url, "index_fonctions.html"))
urls <- c(urls, paste0(base_url, "index_extensions.html"))
cat(urls, sep="\n", file="sitemap.txt")
```

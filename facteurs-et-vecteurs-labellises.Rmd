...
Jeton un œil à la liste des variables de `d` :
...
## Les vecteurs labellisés {#labelled}
...
<div class="important">
L'extension `labelled`{.pkg} est récente et pas encore disponible sur **CRAN**. On devra donc l'installer à partir de **GitHub** avec la commande suivante :

```{r, eval=FALSE}
library(devtools)
install_github("larmarange/labelled")
```
...
Ls <dfn data-index="étiquette de variable">étiquettes de variables</dfn><dfn data-index="variable, étiquette"></dfn> permettent de donner un nom long, plus explicite, aux différentes colonnes d'un tableau de données (ou encore directement à un vecteur autonome).

La visonneuse de données de **RStudio** sait reconnaître et afficher ces étiquettes de variables lorsqu'elles existent. Essayez par exemple la commande suivante :
...
var_label(femmes$id_menage) <- "ID du ménage à laquelle est appartient"
var_label(femmes$id_menage)
```
...
On peut associer une étiquette à n'importe quel type de variable, qu'elle soit numérique, textuelle, un facteur ou encore des dates.
...
Lorsqu'un vecteur possède des étiquettes de valeur, sa classe change et devient `labelled`{data-pkg="labelled"}. Regardons déjà quelques exemples. Tout d'abord, jetons un apercu contenu de l'objet `femmes` grace à la fonction `glimpse`{data-pkg="dplyr"} de l'extension `dplyr`{.pkg}.
...
Notre vecteur `v` a automatiquement transformé en un vecteur de la classe `labelled`. Mais son type sous-jacent est resté `"character"`. Par ailleurs, les données elle-même n'ont pas été modifiées et ont conservé leurs valeurs originales.
...
Un facteur stocke de manière interne les valeurs sous la forme d'une suite d'entiers, démarrant toujours par 1 et forcéments consécutifs, et dont les valeurs dépendent de l'ordre des facteurs. Pour s'en rendre compte :
...
### Définir des valeurs manquantes

Nous avons que sous **R** nous pouvions indiquer des valeurs manquantes avec `NA`. Or, dans le domaine des enquêtes, il arrive fréquemment que l'on ait besoin de pouvoir spécifier plusieurs types de valeurs manquantes. En effet, le fait que l'enquêté ne sache pas quoi répondre à une question n'est pas la même chose qu'un refus de répondre ou encore qu'un oubli de répondre (i.e. aucune case n'a été cochée sur le questionnaire papier). Il est alors fréquent dans ces enquêtes d'utiliser différents codes pour indiquer ces cas de figure. Les habitués de **SPSS** savent qu'il est possible dans ce logiciel d'indiquer quelles valeurs spécifiques doivent être considérées comme manquantes.

La fonction `missing_val`{data-pkg="labelled"} de l'extension `labelled`{.pkg} permet de spécifier quelles valeurs doivent être considérées comme manquantes. Prenons un exemple :

```{r}
v <- c(1, 2, 1, 8, 3, 9)
val_labels(v) <- c(oui = 1, "peut-être" = 2, non = 3, "ne sait pas" = 8, refus = 9)
v
```

À ce stade, aucune valeur manquante n'a encore été définie. Ce que l'on peut voir dans la colonne `is_na` qui ne contient que `FALSE`.
...
Nous avons maintenant indiqué que 8 et 9 doivent être considérées comme manquantes. La classe `labelled`{data-pkg="labelled"} impose qu'une valeur manquante ait une étiquette de valeur. Si l'on essaie d'attribuer une valeur manquante qui n'a pas d'étiquette, on obtient un message d'erreur.
...
On peut indiquer avec l'argument `force` ce que l'on souhaite faire dans ces cas-là : `FALSE` si l'on souhaite ne pas tenir compte des valeurs manquantes qui n'ont pas d'étiquette (et ne seront donc pas considérées comme maquantes) ou `TRUE` pour créer automatiquement une étiquette à ces valeurs qui n'en ont pas encore une.
...
À cette étape, nous n'avons fait qu'indiquer que ces valeurs devraient être considérées comme manquantes. Pour autant, elles sont toujours présentes dans notre vecteur et considérées comme des valeurs valides par les fonctions statistiques de **R**.
...
L'[import de données labellisées](import-de-donnees.html) et le [recodage de variables](recodage.html) (dont la conversion d'un vecteur labellisé en facteur) seront quant à eux abordés dans de prochains chapitres.

...
d'estimation de modèles linéaires (`lm`{data-package="stats"}) ou de modèles linéaires généralisés 
(`glm`{data-package="stats"}), ou dans les analyses de correspondances^[Voir le chapitre dédié à 
l'analyse des correspondances, MAJ LIEN.] des extensions <strong class="package">ade4</strong>
ou <strong class="package">FactoMineR</strong>.

Par contre cette option n'est pas présente dans les fonctions de base comme `mean`{data-package="base"},
`var`{data-package="base" data-rdocumentation="cor"}, `table`{data-package="base"} ou
`chisq.test`{data-package="stats"}.

## Fonctions de l'extension questionr

L'extension <strong class="package">questionr</strong> propose quelques fonctions permettant de calculer 
des statistiques simples pondérées^[Les fonctions `wtd.mean`{data-package="questionr"} et 
`wtd.var`{data-package="questionr" data-rdocumentation="wtd.mean"} sont des copies conformes des fonctions 
du même nom de l'extension <strong class="package">Hmisc</strong> de Frank Harrel. 
<strong class="package">Hmisc</strong> étant une extension « de taille », on a préféré recopié les fonctions 
pour limiter le poids des dépendances.] :

* `wtd.mean`{data-package="questionr"} : moyenne pondérée
* `wtd.var`{data-package="questionr" data-rdocumentation="wtd.mean"} : variance pondérée
* `wtd.table`{data-package="questionr"} : tris à plat et tris croisés pondérés
...
des tableaux croisés (les fonctions `lprop`{data-package="questionr" data-rdocumentation="rprop"}, 
`cprop`{data-package="questionr"} ou `chisq.test`{data-package="stats"} peuvent être appliquées au résultat 
d'un `wtd.table`{data-package="questionr"}) mais restent limitées en termes de tests statistiques 
ou de graphiques. . .

## Données pondérées avec l'extension survey

L'extension <strong class="package">survey</strong> est spécialement dédiée au traitement d'enquêtes 
...
Pour utiliser les fonctionnalités de l'extension, on doit d'abord définir le *plan d'échantillonnage*
ou *design* de notre enquête. C'est-à-dire indiquer quel type de pondération nous souhaitons lui appliquer. 

Dans notre cas nous utilisons plan d'échantillonnage le plus simple, avec une variable de pondération 
déjà calculée. Ceci se fait à l'aide de la fonction `svydesign`{data-package="survey"} :
...
* `svymean`{data-package="survey" data-rdocumentation="surveysummary"}, 
  `svyvar`{data-package="survey" data-rdocumentation="surveysummary"},
  `svytotal`{data-package="survey" data-rdocumentation="surveysummary"}, 
  `svyquantile`{data-package="survey"} : statistiques univariées
* `svytable`{data-package="survey"} : tableaux croisés
* `svychisq`{data-pacakge="survey" data-rdocumentation="svytable"} : test du &chi;²
* `svyby`{data-package="survey"} : statistiques selon un facteur
* `svyttest`{data-package="survey"} : test *t* de comparaison de moyennes
* `svyglm`{data-pacakge="survey"} : modèles linéaires généralisés (dont régression logistique)
* `svyplot`{data-package="survey"}, 
  `svyhist`{data-package="survey"}, 
  `svyboxplot`{data-package="survey" data-rdocumentation="svyhist"} : fonctions graphiques

D'autres fonctions sont disponibles, comme `svyratio`{data-package="survey"}, 
...
L'intervalle de confiance d'une moyenne s'obtient avec 
`confint`{data-package="survey" data-rdocumentation="surveysummary"}
et celui d'une proportion avec `svyciprop`{data-package="survey"}.
...
Pour comparer deux moyennes à l'aide d'un test *t* on aura recours à `svyttest`{data-package="survey"} :
...
On peut récupérer le tableau issu de `svytable`{data-package="survey"} dans un objet et 
...
Les fonctions `lprop`{data-package="questionr" data-rdocumentation="rprop"} et 
`cprop`{data-package="questionr"} de <strong class="package">questionr</strong> sont donc 
tout à fait compatibles avec l'utilisation de <strong class="package">survey</strong>. 

```{r}
lprop(tab)
```

La fonction `freq`{data-package="questionr"} peut également être utilisée si on lui passe 
en argument non pas la variable elle-même, mais son tri à plat obtenu avec 
`svytable`{data-package="survey"}svychisq(~sexe + clso, dw):
...
Par contre, il ne faut pas utiliser `chisq.test`{data-package="stats"} sur un tableau généré 
par `svytable`{data-package="survey">. Les effectifs étant extrapolés à partir de la pondération, 
...
`svychisq`{data-package="survey" data-rdocumentation="svytable"} :

```{r}
svychisq(~sexe + clso, dw)
```

Le principe de la fonction `svyby`{data-pacakge="survey"} est similaire à celui de 
`tapply`{data-package="base"}^[La fonction `tapply`{data-package="base"} est présentée plus en 
...
<strong class="package">survey</strong> est également capable de produire des graphiques à partir 
...
Enfin, <strong class="package">survey</strong> fournit une fonction `svyglm`{data-package="survey"}
permettant de calculer un modèle statistique tout en prenant
en compte le plan d'échantillonnage spécifié. La syntaxe de `svyglm`{data-package="survey"} est proche
de celle de `glm`{data-package="stats"}. Cependant, le cadre d'une régression logistique,
...
reg
```
Le résultat obtenu est similaire à celui de `glm`{data-package="stats"} et l'on peut utiliser 
sans problème les fonctions `coef`{data-package="survey" data-rdocumentation="svyglm"},
`confint`{data-package="survey" data-rdocumentation="confint.glm"}, 
`odds.ratio`{data-package="questionr"} ou `predict`{data-package="survey" data-rdocumentation="svyglm"}
...
Dans ses dernières versions, <strong class="package">survey</strong> fournit une méthode 
`AIC.svyglm`{data-package="survey" data-rdocumentation="anova.svyglm"} permettant d'estimer un AIC 
sur un modèle calculé avec `svyglm`{data-package="survey"}. Il est dès lors possible d'utiliser 
la fonction `step`{data-package="stats"} pour réaliser une *sélection descendante pas à pas*.

L'extension <strong class="package">effects</strong> n'est quant à elle pas compatible avec
`svyglm`{data-package="survey"}^[Compatibilité qui pourra éventuellement être introduite dans une future version de l'extension.].

## Définir un plan d'échantillonage complexe avec survey

L'extension <strong class="package">survey</strong> ne permet pas seulement d'indiquer une 
...
homogènes (strates) pour ensuite extraire un échantillon aléatoire de chaque strate. Cette méthode suppose
...
L'*échantillonnage par grappes* est une méthode qui consiste à choisir un échantillon aléatoire d'unités
qui sont elles-mêmes des sous-ensembles de la population (« grappes »). Cette méthode suppose que les
unités de chaque grappe sont représentatives. Elle possède l'avantage d'être souvent plus économique.

Il est possible de combiner plusieurs de ces approches. Par exemple, les *Enquêtes Démographiques et
de Santé^[Vaste programme d'enquêtes réalisées à intervalles réguliers dans les pays en dévelop'ent,
disponibles sur <http://www.dhsprogram.com/>.] (EDS) sont des enquêtes stratifiées en grappes à deux degrés. 
...
La fonction `svydesign`{data-package="survey"} accepte plusieus arguments décrits sur sa page 
...
écrira alors pour l'argument `ids` la valeur suivante : `ids=˜id_departement+id_commune+id_quartier`.
...
Si V022 n'est pas fourni mais que l'enquête a bien été stratifié par région et milieu de résidence (vérifiez
toujours le premier chapitre du rapport d'enquête), on pourra créer une variable strate ainsi
^[L'astuce consiste à utiliser as.integer pour obetnir le code des facteurs et non leur valeur textuelle. 
...
Si l'on souhaite travailler sur un sous-échantillon tout en gardant les informations d'échantillonnage,
on utilisera la fonction `subset`{data-package="survey" data-rdocumentation="subset.survey.design"}
...
* utiliser les options de pondération des fonctions usuelles ou les fonctions d'extensions 
  comme <strong class="package">questionr</strong> pour les cas les plus simples ;
* si on utilise <strong class="package">survey</strong>, effectuer autant que possible tous les 

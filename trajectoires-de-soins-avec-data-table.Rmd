---
title: "Trajectoires de soins : un exemple de données longitudinales avec data.table"
---
...
<div class="note">
Ceci est une version du chapitre [trajectoires de soins](trajectoires-de-soins.html) utilisant l'extension `data.table`{.pkg} plutôt que `dplyr`{.pkg}.
</div>
...
Nous allons donc réaliser une analyse de séquences (voir le [chapitre dédié](analyse-de-sequences.html)) sur l'ensemble de notre fichier. Pour cela, il va falloir préalable que nous transformions nos donnée actuellement dans un format <q>long</q> en un tableau <q>large</q>, c'est-à-dire avec une ligne par individu et une variable différentes par pas de temps. On peut réaliser cela facilement avec `pivot_wider`{data-pkg="tidyr"} de `tidyr`{.pkg} (voir le [chapitre dédié à **tidyr**](reorganiser-ses-donnees-avec-tidyr.html#spread-disperser-des-lignes)). Il faut noter que le résultat ne sera pas un tableau **data.table**, d'où le recours à `setDT`{data-pkg="data.table"} pour convertir le résultat.

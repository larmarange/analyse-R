...
Dès lors, plutôt que d'éparpiller les appels à `library`{data-pkg="base"} dans vos scripts, de les regrouper
au début de ces derniers.
...
étiquettes de variables. C'est le cas par exemple du tableau `iris`. `iris` est un tableau
de données directement accessibles dans **R** et souvent utilisé pour les exemples de la 
...
Cette section sera développée ultérieurement, suite aux discussions sur
<https://github.com/juba/questionr/issues/53>.

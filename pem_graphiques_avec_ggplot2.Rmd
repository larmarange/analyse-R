...
Il faut noter que `ggplot2`{.pkg} a ici réordonné les pays par ordre alphabétique, et que la visualisation par ligne tend à rendre plus difficile les périodes pour lesquelles il manque des données (voir, toujours, la ligne pour l'Espagne). Le résultat est toutefois assez lisible, et donne à voir les couleurs utilisées par défaut par les graphiques de `ggplot2`{.pkg}.

Il faut aussi noter que `ggplot2`{.pkg} utilise une syntaxe additive, où différents éléments et paramètres graphiques peuvent être combinés en les additionnant, ce qui permet de construire et de modifier des graphiques de manière cumulative, pas à pas. Cette caractéristique permet de tâtonner, et de construire progressivement des graphiques très complets.

### Combinaisons d'éléments graphiques

On n'a pas encore visualisé le ratio <q>Dette publique / PIB</q>, l'autre variable du raisonnement de Reinhart et Rogoff. C'est l'occasion de voir comment rajouter des titres aux axes des graphiques de `lattice`{.pkg}, et d'utiliser les lignes en même temps que des points, grâce à l'argument `type`, qui peut prendre plusieurs valeurs (ici, `"o"` produit les points et `"l"` produit les lignes) :
...
Les différents exemples vus ci-dessus demandent d'apprendre un minimum de syntaxe graphique en se familiarisant soit avec `lattice`{.pkg}, soit avec `ggplot2`{.pkg}. Ce petit investissement permet de savoir très vite produire de très nombreux graphiques, assez élégants de surcroît. La suite du texte se concentre sur l'apprentissage de la syntaxe de `ggplot2`{.pkg}.
...
Les graphiques produits par `ggplot2`{.pkg} peuvent être sauvegardés manuellement, comme expliqué dans le chapitre <q>[Exporter des graphiques](pem_export_graphiques.html)</q>, ou programmatiquement. Pour sauvegarder le dernier graphique affiché par `ggplot2`{.pkg} au format PNG, il suffit d'utiliser la fonction `ggsave`{data-pkg="ggplot2" data-rdoc="ggsave"}, qui permet d'en régler la taille (en pouces) et la résolution (en pixels par pouce ; 72 par défaut) :
...
On trouve aussi très facilement, ailleurs sur Internet, des dizaines de _tutorials_ et autres _cheatsheets_ pour `ggplot2`{.pkg}, [ici](http://zevross.com/blog/2014/08/04/beautiful-plotting-in-r-a-ggplot2-cheatsheet-3/) ou [là](http://www.rstudio.com/wp-content/uploads/2015/04/ggplot2-cheatsheet.pdf) par exemple. Enfin, il faut signaler quelques-unes des différentes extensions que l'extension a inspirées, dont la plupart sont encore en cours de développement :

- l'extension <strong class="package">ggfortify</strong> permet de [visualiser les résultats de différentes fonctions de modélisation](https://github.com/sinhrks/ggfortify/blob/master/README.rst#examples) avec `ggplot2`{.pkg} ;
- l'extension <strong class="package">ggmap</strong> permet de visualiser des fonds de carte et d'y superposer des éléments graphiques rédigés avec `ggplot2`{.pkg} ;
- l'extension <strong class="package">GGally</strong> rajoute quelques types de graphiques à ceux que `ggplot2`{.pkg} peut produire par défaut ;
- et des extensions comme <strong class="package">ggvis</strong> permettent de [produire des graphiques interactifs](http://ggvis.rstudio.com/) en utilisant la syntaxe de base de `ggplot2`{.pkg}.

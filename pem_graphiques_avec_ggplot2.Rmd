...
**R** est distribué avec une extension qui permet de moins avoir à bricoler les graphiques : <strong class="package">lattice</strong>. Mais surtout, **R** est aussi capable de comprendre la <q>grammaire graphique</q> [théorisée par Leland Wilkinson](https://books.google.fr/books/about/The_Grammar_of_Graphics.html), et traduite pour **R** par Hadley Wickham sous la forme de l'extension <strong class="package">ggplot2</strong>^[Voir l'excellente [documentation de l'extension](http://ggplot2.org/) et les autres ressources citées en fin de texte.]. Ce chapitre présente ces deux extensions à partir d'un exemple simple de visualisation de séries temporelles, puis rentre dans le détail de la syntaxe de <strong class="package">ggplot2</strong>^[Bien que l'on ait fait le choix de présenter le détail de l'extension <strong class="package">ggplot2</strong> plutôt que de l'extension <strong class="package">lattice</strong>, ce dernier reste un excellent choix pour la visualisation, notamment, de [panels](http://www.magesblog.com/2013/09/using-planelgroups-in-lattice.html) et de [séries temporelles](http://www.fromthebottomoftheheap.net/2013/10/23/time-series-plots-with-lattice-and-ggplot/). On trouve de [très beaux exemples](https://procomun.wordpress.com/2015/04/14/mapping-flows-in-r-with-data-table-and-lattice/) d'utilisation de <strong class="package">lattice</strong> en ligne, mais un peu moins de documentation, et beaucoup moins d'extensions, que pour <strong class="package">ggplot2</strong>.].
...
Chargeons à présent les deux extensions graphiques dont nous allons nous servir dans cet exemple :
...
Le même graphique s'écrit de la mnière suivante avec l'extension <strong class="package">lattice</strong> :
...
La syntaxe de cet exemple est redevenue : <q>avec le jeu de données `debt`, visualiser les variables <var>Year</var> sur l'axe `x` et <var>growth</var> sur l'axe `y`</q>. Il faut aussi noter que, dans les deux exemples précédents, on a écrit `with(debt, ...)` pour indiquer que l'on travaillait avec l'objet `debt`, mais les extensions <strong class="package">lattice</strong> et <strong class="package">ggplot2</strong> permettent aussi d'utiliser l'argument `data` pour ce faire :
...
<div class="note">
Le paramètre `facet_grid`{data-pkg="ggplot2" data-rdoc="facet_grid"}, qui utilise aussi la notation <q>équation</q>, permet de créer des facettes plus compliquées que celles créées par le paramètre `facet_wrap`{data-pkg="ggplot2" data-rdoc="facet_wrap"}, même si, dans nos exemples, on aurait pu utiliser aussi bien l'un que l'autre.
</div>
...
Ce chapitre n'a pu faire la démonstration que d'une infime partie des manières d'utiliser <strong class="package">ggplot2</strong>. En voici une dernière illustration, qui donne une idée des différents types de graphiques que l'extension permet de produire dès que l'on connaît les principaux éléments de sa syntaxe :
...
Pour tout ce qui concerne l'utilisation de <strong class="package">ggplot2</strong>, [l'ouvrage de Wickham](http://ggplot2.org/book/), [en cours d'actualisation](https://github.com/hadley/ggplot2-book), est la ressource essentielle à consulter. [L'ouvrage de Winston Chang](https://books.google.fr/books?id=ob0VAAAAQBAJ), qui contient des [dizaines d'exemples](http://www.cookbook-r.com/Graphs/), le complète utilement, de même que la [documentation en ligne](http://docs.ggplot2.org/current/) de l'extension. Enfin, le site de questions/réponses [StackOverflow](https://stackoverflow.com/questions/tagged/ggplot2) contient de très nombreuses discussion des subtilités de sa syntaxe.

On trouve aussi très facilement, ailleurs sur Internet, des dizaines de _tutorials_ et autres _cheatsheets_ pour <strong class="package">ggplot2</strong>, [ici](http://zevross.com/blog/2014/08/04/beautiful-plotting-in-r-a-ggplot2-cheatsheet-3/) ou [là](http://www.rstudio.com/wp-content/uploads/2015/04/ggplot2-cheatsheet.pdf) par exemple. Enfin, il faut signaler quelques-unes des différentes extensions que l'extension a inspirées, dont la plupart sont encore en cours de développement :

- l'extension <strong class="package">ggfortify</strong> permet de [visualiser les résultats de différentes fonctions de modélisation](https://github.com/sinhrks/ggfortify/blob/master/README.rst#examples) avec <strong class="package">ggplot2</strong> ;
- l'extension <strong class="package">ggmap</strong> permet de visualiser des fonds de carte et d'y superposer des éléments graphiques rédigés avec <strong class="package">ggplot2</strong> ;
- l'extension <strong class="package">GGally</strong> rajoute quelques types de graphiques à ceux que <strong class="package">ggplot2</strong> peut produire par défaut ;
- et des extensions comme <strong class="package">ggvis</strong> permettent de [produire des graphiques interactifs](http://ggvis.rstudio.com/) en utilisant la syntaxe de base de <strong class="package">ggplot2</strong>.

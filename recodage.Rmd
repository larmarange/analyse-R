...
<strong class="pkg">forcats</strong> est une extension facilitant la manipulation des variables qualitatives, qu'elles soient sous forme de vecteurs `character` ou de facteurs. Elle fait partie du **tidyverse**, et est donc automatiquement chargée par :
...
Il existe plusieurs possibilités pour effectuer ce type de recodage, mais ici on va utiliser la fonction `fct_recode`{data-pkg="forcats"} de l'extension <strong class="pkg">forcats</strong>. Celle-ci prend en argument une liste de recodages sous la forme `"Nouvelle valeur" = "Ancienne valeur"`. 
...
D'autres fonctions sont proposées par <strong class="forcats">forcats</strong> pour faciliter certains recodage, comme `fct_collapse`{data-pkg="forcats"}, qui propose une autre syntaxe pratique quand on doit regrouper ensemble des modalités :

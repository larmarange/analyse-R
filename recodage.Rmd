...
<div class="guide-R">
Une version actualisée de ce chapitre est disponible sur **guide-R** : [Facteurs et forcats](https://larmarange.github.io/guide-R/manipulation/facteurs.html) & [Combiner plusieurs variables](https://larmarange.github.io/guide-R/manipulation/combiner-variables.html)
</div>
...
Si l'on utilise `dplyr`{.pkg} (voir [chapitre dédié](manipuler-les-donnees-avec-dplyr.html)), on peut facilement renommer une variable avec `rename`{data-pkg="dplyr"} :
...
L'extension `cleaner`{.pkg} propose plusieurs fonctions utiles lorsque l'on doit nettoyer une variable mal encodées.

```{r}
library(cleaner)

# Recoder en valeurs logiques
...
<div class="important">
L'extension `cleaner`{.pkg} propose également une fonction `freq`{data-pkg="cleaner"} qui peut rentrer en conflit avec la fonction `freq`{data-pkg="questionr"} de `questionr`{.pkg}. Dans ce cas, on pourra soit appeler explicitement la version de `questionr`{.pkg} avec l'écriture `questionr::freq()`. Une autre possibilité est de <q>décharger</q> (le contraire de charger en mémoire) l'extension `cleaner`{.pkg} une fois que l'on en n'a plus besoin.

```{r}
detach("package:cleaner", unload = TRUE)
```

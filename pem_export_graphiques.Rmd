...
L'export de graphiques est très facile avec **RStudio**. Lorsque l'on
...
La boîte de dialogue qui s'ouvre permet différentes options d'export :
...
les principaux. Les formats **PNG**, **JPEG** et **TIFF** sont des formats de type *bitmap*
(on parle aussi d'*images matricieles*^[Voir <http://fr.wikipedia.org/wiki/Image_matricielle>.]). 
L'image est stockée sous forme de points, sa qualité dépendant de sa résolution, c'est-à-dire du 
...
Les *images vectorielles*^[Voir <http://fr.wikipedia.org/wiki/Image_vectorielle>.] ont l'avantage 
...
directement être lus par la majorité des navigateurs récents (**Firefox**, **Chrome**, ...).

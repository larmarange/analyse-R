...
Chargeons les mêmes données que le [chapitre sur les graphiques bivariés](analyser_intro_graph_biv.html).
...
La notation <q>formule</q> devient cruciale dès que l'on souhaite rédiger des modèles : la formule `y ~ x`, par exemple, qui est équivalente à la formule `y ~ 1 + x`, correspond à l'équation mathématique $Y = a + bX$. On trouvera de nombreux exemples d'usage de cette notation dans les chapitres consacrés, notamment, à la régression linéaire ou à la [régression logistique](analyser_inter_reg_log.html).

De la même manière, l'opérateur `|` (_pipe_) utilisé par l'extension `lattice`{.pkg} joue aussi un rôle très important dans la rédaction de modèles multi-niveaux, où il sert à indiquer les variables à pentes ou à coefficients aléatoires. Ces modèles sont présentés dans un [chapitre dédié](analyser_avance_effets_mixtes.html).

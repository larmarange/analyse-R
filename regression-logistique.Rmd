...
Il est possible de représenter graphiquement les différents odds ratios. Pour cela, on va transformer le résultat de `odds.ratio`{data-pkg="questionr"} en tableau de données, renommer les colonnes pour plus de commodité et stocker le nom des modalités dans une variable. `geom_errorbar`{data-pkg="ggplot2" data-rdoc="geom_crossbar"} permets de représenter des barres d'erreurs, `geom_hline`{data-pkg="ggplot2" data-rdoc="geom_abline"} une ligne au niveau `y = 1`, `scale_y_log10`{data-pkg="ggplot2" data-rdoc="scale_continuous"} pour afficher l'axe des `y` de manière logarithmique, les odds ratios étant de nature multiplicative et non additive et `coord_flip`{data-pkg="ggplot2"} pour inverser l'axe des `x` et celui des `y` afin de faciliter la lecture du graphique.

<figure>
```{r}
tmp <- as.data.frame(odds.ratio(reg))
str(tmp)
names(tmp) <- c("OR", "ICmin", "ICmax", "p")
tmp$modalite <- row.names(tmp)
str(tmp)
library(ggplot2)
ggplot(tmp) + 
  aes(x = modalite, y = OR, ymin = ICmin, ymax = ICmax) +

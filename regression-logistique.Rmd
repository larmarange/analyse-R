...
Si vous utilisez les rapports **Rmarkdown** (voir le [chapitre dédié](rmarkdown-les-rapports-automatises.html)), ce tableau peut être affiché avec la fonction `kable`{data-pkg="knitr"} de l'extension `knitr`{.pkg}.

```{r}
knitr::kable(tmp)
```

La fonction `tbl_regression`{data-pkg="gtsummary"} de l'extension `gtsummary`{.pkg}, qui a recours en interne à `tidy`{data-pkg="broom" data-rdoc="gm_tidiers"}, permet d'obtenir un tableau plus propre. Comme nous souhaitons afficher les *odds ratios* plutôt que les coefficients du modèle, on indiquera `exponentiate = TRUE`.
...
Comme `tbl_regression`{data-pkg="gtsummary"} respose sur l'extension `broom`{.pkg}, `gtsummary`{.pkg} peut en principe couvrir un plus grand nombre de modèles que `finalfit`{.pkg}.
...
```{r}
tbl_summary(
  d[, c("sport", "sexe", "grpage", "etud", "relig", "heures.tv")]
)
```

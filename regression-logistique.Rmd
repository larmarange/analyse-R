...
<div class="note">
L'extension `JLutils`{.pkg}, disponible uniquement sur GitHub, propose deux fonctions intéressantes dans ce contexte. Pour l'installer ou la mettre à jour, si ce n'est déjà fait, on aura recours à la commande ci-après.

```{r, eval=FALSE}
devtools::install_github("larmarange/JLutils")
```

La fonction `ggcoef`{data-pkg="JLutils"} permet d'effectuer le graphique précédent directement à partir de notre modèle. Voir l'aide de cette fonction pour la liste complète des paramètres personnalisables.
...
Il est possible de combiner les deux pour personnaliser un peu plus le résultat.

...
<div class="guide-R">
Une version actualisée de ce chapitre est disponible sur **guide-R** : [Régression logistique binaire](https://larmarange.github.io/guide-R/analyses/regression-logistique-binaire.html). Voir également [Sélection pas à pas d’un modèle](https://larmarange.github.io/guide-R/analyses/selection-modele-pas-a-pas.html), [Régression logistique multinomiale](https://larmarange.github.io/guide-R/analyses_avancees/regression-logistique-multinomiale.html), [Régression logistique ordinale](https://larmarange.github.io/guide-R/analyses_avancees/regression-logistique-ordinale.html) et [Régression logistique binaire pondérée](https://larmarange.github.io/guide-R/donnees_ponderees/regression-logistique-binaire-ponderee.html).
</div>
...
`fct_relevel`{data-pkg="forcats"} ou la fonction
...
comme une modalité supplémentaire. Auquel cas, nous utiliserons la fonction `fct_explicit_na`{data-pkg="forcats"} fournie par `questionr`{.pkg}^[Il existe également une fonction `add.NA`{data-pkg="base"} fournie de base avec **R** et `addNAstr`{data-pkg="questionr"} fournie par `questionr`{.pkg}.] :
...
On pourra se référer à la [vignette dédiée (en anglais)](https://www.danieldsjoberg.com/gtsummary/articles/tbl_regression.html) pour découvrir les différentes options de `tbl_regression`{data-pkg="gtsummary"}.
...
<div class="guide-R">
Une version actualisée de cette section est disponible sur **guide-R** : [Prédictions marginales, contrastes marginaux & effets marginaux](https://larmarange.github.io/guide-R/analyses/estimations-marginales.html)
</div>
...
Un critère similaire à l'<dfn>AIC</dfn> est le critère <dfn>BIC</dfn> (<dfn lang="en">Bayesian Information Criterion</dfn>) appelé aussi <dfn>SBC</dfn> (<dfn lang="en">Schwarz information criterion</dfn>). Il s'obtient avec `step`{data-pkg="stats"} an ajoutant l'argument `k = log(n)` où `n` est le nombre d'observations inclues dans le modèle que l'on peut obtenir avec `nrow(model.matrix(reg))` (pour tenir compte des éventuelles observations manquantes retirées des données pour le calcul du modèle).

```{r}
reg2_bic <- step(reg, k = log(nrow(model.matrix(reg))))
```
...
La fonction `tbl_uvregression`{data-pkg="gtsummary"} permets quant à elle de réaliser plusieurs régressions univariées et de les présenter dans un tableau unique.
...
Il est possible de réunir analyse univariée et multivariée en un seul table avec `tbl_merge`{data-pkg="gtsummary"}.
...
<div class="webin-R">
La régression logistique multinomiale est évoquée dans le webin-R #17 (trajectoires de soins : un exemple de données longitudinales) sur [YouTube](https://youtu.be/JV1Srrg09oI).
</div>
...
La plupart des fonctions vues précédemment fonctionnent, y compris `tbl_regression`{data-pkg="gtsummary"} (par contre, `add_global_p`{data-pkg="gtsummary"} n'est pour le moment pas compatible)  :
...
Pour une bonne introduction à l'extension `ordinal`{.pkg}, on pourra se référer au tutoriel officiel (en anglais) : <https://cran.r-project.org/web/packages/ordinal/vignettes/>.

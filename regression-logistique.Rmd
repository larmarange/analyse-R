...
Par défaut, toutes les variables explicatives fournies sont retenues dans le modèle affiché. Si on ne souhaite inclure que certaines variables dans le modèle mutivarié (parce que l'on aura précédemment réalisé une procédure `setp`{data-pkg="stats"}), il faudra préciser séparément les variables du modèle multivarié.
...
`svrepmisc`{.pkg} fournit également des méthodes `confint`{data-pkg="svrepmisc" data-rdocumentation="svymultinom"} et `tidy`{data-pkg="svrepmisc" data-rdoc="tidy.svrepstatmisc"}. Il est également possible d'utiliser `ggcoef`{data-pkg="GGally"}.

...
<div class="note">
Il est possible de combiner `tidy_plus_plus`{data-pkg="broom.helpers"} avec `ggcoef`{data-pkg="GGally"} pour personnaliser un peu plus le résultat.
...
td$var_label <- stringr::str_wrap(td$var_label, 20)
ggcoef(
  td, 
...
Une version améliorée de `ggcoef`{data-pkg="GGally"} est en cours de développement et sera prochainement disponible dans `GGally`{.pkg}
</div>
...
A la différence de `ggcoef`{data-pkg="GGally"} qui fonctionne avec la plupart des types de modèles (dont les modèles à effets mixtes ou ceux réalisés avec un plan d'échantillonnage complexe), `forest_model`{data-pkg="forestmodel"} ne fonctionne qu'avec les modèles les plus courants.
...
La méthode `tidy`{data-pkg="broom"} étant disponible, on peut utiliser `ggcoef`{data-pkg="GGally"} et `tbl_regression`{data-pkg="gtsummary"}.

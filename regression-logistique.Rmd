...
L'extension la plus utilisée pour réaliser des modèles ordinaux est <strong class="pkg">ordinal</strong> et sa fonction `clm`{data-pkg="ordinal"}. Il est même possible de réaliser des modèles ordinaux avec des [effets aléatoires](modeles-a-effets-aleatoires.html) (modèles mixtes) à l'aide de la fonction `clmm`{data-pkg="ordinal"}.

Pour une bonne introduction à l'extension <strong class="pkg">ordinal</strong>, on pourra se référer au tutoriel officiel (en anglais) : <https://cran.r-project.org/web/packages/ordinal/vignettes/clm_tutorial.pdf>.

Une autre introduction pertinente (en français) et utilisant cette fois-ci l'extention <strong class="pkg">VGAM</strong> et sa fonction `vglm`{data-pkg="VGAM"} est disponible sur le site de l'université de Lyon : <https://eric.univ-lyon2.fr/~ricco/cours/didacticiels/data-mining/didacticiel_Reg_Logistique_Polytomique_Ordinale.pdf>.
...
L'extension <strong class="pkg">broom</strong> ne propose pas de méthode `tidy`{data-pkg="broom"} pour les objets `clm`{data-pks="ordinal"}. Cependant, on pourra en trouver une dans l'extension <strong class="pkg">JLutils</strong>. Pour rappel, cette extension est disponible uniquement sur [GitHub](https://github.com/larmarange/JLutils) et s'installe donc ainsi :
...
La méthode `tidy.clm`{data-pkg="JLutils" data-rdocumentation="tidy_model"} étant disponible, on peut utiliser `ggcoef`{data-pkg="GGally"}.
...
Lorsque l'on utilise des données pondérées, on aura recours à l'extension <strong class="pkg">survey</strong>^[Voir le chapitre dédié aux [données pondérées](donnees-ponderees.html#survey).].
...
L'extension <strong class="pkg">survey</strong> fournit une fonction `svyglm`{data-pkg="survey"}
...
<strong class="pkg">svrepmisc</strong> fournit une fonction `svymultinom`{data-pkg="svrepmisc"} pour le calcul d'une régression multinomiale avec des poids de réplication.
...
<strong class="pkg">svrepmisc</strong> fournit également des méthodes `confint` et `tidy`{data-pkg="svrepmisc" data-rdocumentation="tidy.svrepstatmisc"}. Il est également possible d'utiliser `ggcoef`{data-pkg="GGally"}.
...
Pour un modèle ordinal, il existe une version simplifiée des modèles ordinaux directement disponible dans <strong class="pkg">survey</strong> via la fonction `svyolr`{data-pkg="survey"}.
...
L'extension <strong class="pkg">JLutils</strong>^[`devtools::install_github("larmarange/JLutils")` pour l'installer] propose une méthode `tidy`{data-pkg="JLutils" data-rdocumentation="tidy_model"} pour les objets `svyolr.`
...
Une alternative est d'avoir recours, comme pour la régression multinomiale, aux poids de réplication et à la fonction `svyclm`{data-pkg="svrepmisc"} implémentée dans l'extension <strong class="pkg">svrepmisc</strong>.

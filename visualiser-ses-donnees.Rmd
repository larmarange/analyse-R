...
L'extension `dplyr`{.pkg} (voir le [chapitre dédié](introduction-a-dplyr.html)), propose une fonction `glimpse`{data-pkg="tibble"} (ce qui signifie <q>aperçu</q> en anglais) qui permet de visualiser rapidement et de manière condensée le contenu d'un tableau de données.
...
La fonction `lookfor`{data-pkg="questionr"} est par ailleurs compatible avec les étiquettes de variable de l'extension <strong class="pkg">labelled</strong>, les étiquettes étant prise en compte dans la recherche d'une variable.
...
Il est possible de sélectionner des variables à la manière de <strong class="pkg">dplyr</strong>. Voir l'aide de `contains`{data-pkg="dplyr" data-rdocumentation="select_helpers"}.
...
L'extension <strong class="pkg">codebook</strong> est en cours de développements sur [GitHub](https://github.com/rubenarslan/codebook). Pour l'installer, on aura donc recours à la commande suivante.
...
L'extension <strong class="pkg">DataExplorer</strong> fournit des outils d'exploration graphique d'un fichier de données. En premier lieu, sa fonction `create_report`{data-pkg="DataExplorer"} génère un rapport automatique à partir d'un tableau de données.
...
L'extension <strong class="pkg">dataMaid</strong> propose une fonction `makeCodebook`{data-pkg="dataMaid"} permettant de générer une présentation de l'ensemble des variables d'un tableau de données, au format PDF, Word ou HTML.

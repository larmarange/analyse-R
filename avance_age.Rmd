...
Le package <strong class="package">lubridate</strong> est spécialement développé pour faciliter la manipulation 
et le calcul autour des dates. La version de développement intègre depuis peu une fonction 
`time_length`{data-package="lubridate"} adaptée au calcul des âges exacts.

<div class="info">
La fonction `time_length`{data-package="lubridate"} étant récente, elle n'est pas 
encore disponible dans la version stable du package. 
Pour installer la version de développement de <strong class="package">lubridate</strong>,
on aura recours au package <strong class="package">devtools</strong> :
...
compte la date anniversaire et le fait que la durée de chaque année (ou mois) est variable. 
C'est justement ce que fait la fonction `time_length`{data-package="lubridate"} appliquée à un objet 
de type `Interval`{data-package="lubridate" data-rdocumentation="Interval-class"}. 
...
Attention, cela n'est valable que si l'on présente à la fonction `time_length`{data-package="lubridate"} 
un objet de type `Interval`{data-package="lubridate" data-rdocumentation="Interval-class"} 
(pour lequel on connait dès lors la date de début et la date de fin). 
Si l'on passe une durée (objet de type `Duration`{data-package="lubridate" data-rdocumentation="Duration-class"})
à la fonction `time_length`{data-package="lubridate"}, le calcul s'effectuera alors en prenant en compte la durée 
...
C'est la position adoptée par la fonction `time_length`{data-package="lubridate"}.
...
Une fois que l'on sait calculer un âge exact, le calcul d'un âge révolu est assez simple. 
...
`Period`{data-package="lubridate" data-rdocumentation="Period-class"} et à ne prendre en compte que les années.
...
Le package <strong class="package">eeptools</strong> fournit de son côté une fonction 
`age_calc`{data-package="eeptools"}^[https://github.com/jknowles/eeptools/blob/master/R/age_calc.R]
...
La méthode utilisée par `age_calc`{data-package="eeptools"} donne des résultats 
légèrement différent de ceux de `time_length`{data-package="lubridate"}. Il est donc conseillé 
d'utiliser de préférence le package <strong class="package">lubridate</strong>.

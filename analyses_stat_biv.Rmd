...
[chapitre dédié](pem_stat_univ.html).

Comme dans la partie précédente, on travaillera sur les jeux de données fournis avec l'extension
<strong class="pkg">questionr</strong> et tiré de l'enquête *Histoire de vie* et du recensement 1999 :
...
à l'aide de la fonction `complete.cases`{data-package="stats"}, 
estimer la densité locale à l'aide de la fonction `kde2d`{data-package="MASS"} de l'extension 
<strong class="pkg">MASS</strong>^[<strong class="pkg">MASS</strong> est installée par défaut 
...
entre les deux variables à l'aide de la fonction `cor`{data-package="stats"} :
...
à l'aide de la fonction `abline`{data-package="graphics"}.
...
à l'aide de la fonction `t.test`{data-package="stats"} :
...
`t.test`{data-package="stats"} propose 
...
avec la fonction `shapiro.test`{data-package="stats"} :
...
fourni par la fonction `var.test`{data-package="stats"} :
...
Une alternative à la fonction `table`{data-package="base"} est la fonction `xtabs`{data-package="stats"}.
...
L'extension <strong class="pkg">questionr</strong>
...
Celle-ci s'applique au tableau croisé généré par `table`{data-package="base"} 
ou `xtabs`{data-package="stats"}  :

```{r}
tab <- table(d$sport, d$qualreg)
lprop(tab)
tab <- xtabs(~ sport + qualreg, d)
lprop(tab)
```
...
`chisq.test`{data-package="stats"}, appliquée au tableau croisé obtenu avec `table`{data-package="base"} 
...
`chisq.residuals`{data-package="questionr"} de <strong class="pkg">questionr</strong> :
...
`cramer.v`{data-package="questionr"} de <strong class="pkg">questionr</strong> :
...
avec la fonction `fisher.test`{data-package="stats"}. On peut soit lui passer le résultat 
de `table`{data-package="base"} ou `xtabs`{data-package="stats"}, 
...
grâce à la fonction `mosaicplot`{data-package="graphics"}.
...
L'extension <strong class="pkg">vcd</strong> fournie une fonction `mosaic`{data-package="vcd"}
...
[statistique univariée](pem_stat_univ.html#ic_proportion)) permets également d'effectuer
...
On pourra également avoir recours à la fonction `fisher.test`{data-package="stats"} qui
...
On pourra aussi avoir recours à la fonction `odds.ratio`{data-package="questionr"} de l'extension
<strong class="pkg">questionr</strong> qui réalise le même calcul mais présente le
...
`relrisk`{data-package="mosaic"} de l'extension <strong class=""package>mosaic</strong>.
